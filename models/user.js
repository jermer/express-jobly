...
class User {
  ...
  static async get(username) {
    ...
    // select job applications for this user
    const appRes = await db.query(
      `SELECT job_id AS "jobId" FROM applications WHERE username = $1`,
      [username]
    );
    // add job application array
    user.jobs = appRes.rows.map(r => r.jobId);
  ...
  /** Record that the given user has applied for the given job id.
   * 
   *  Throws NotFoundError if either the username or the job id are
   *  not found.
   * 
   *  Returns undefined.
   **/
